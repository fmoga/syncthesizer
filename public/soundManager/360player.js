...
function ThreeSixtyPlayer() {
  ...
  this.events = {
    ...
    finish: function() {
      if (threeSixtyPlayer.onfinish) {
    	  threeSixtyPlayer.onfinish();
      }
      var nextLink;
      pl.removeClass(this._360data.oUIBox,this._360data.className);
      this._360data.className = '';
  ...
  this.handleClick = function(e) {
	...
	if (thisSound.paused) {
	  if (thisSound.onplay) thisSound.onplay();
	} else {
	  if (thisSound.onpause) thisSound.onpause();
	}
  ...
  this.mmh = function(e) {
    ...
    var newPos = oSound.durationEstimate*(angle/fullCircle);
    oSound.setPosition(newPos);
    self.stopEvent(e);
    if (oSound.options.onposition) oSound.options.onposition(newPos);
    return false;
